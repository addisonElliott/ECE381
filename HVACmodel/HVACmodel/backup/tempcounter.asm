...
include "m8c.inc"
include "memory.inc"
include "TempCounter.inc"

;-----------------------------------------------
;  Global Symbols
;-----------------------------------------------
export  TempCounter_EnableInt
export _TempCounter_EnableInt
export  TempCounter_DisableInt
export _TempCounter_DisableInt
export  TempCounter_Start
export _TempCounter_Start
export  TempCounter_Stop
export _TempCounter_Stop
export  TempCounter_WritePeriod
export _TempCounter_WritePeriod
export  TempCounter_WriteCompareValue
export _TempCounter_WriteCompareValue
export  TempCounter_wReadCompareValue
export _TempCounter_wReadCompareValue
export  TempCounter_wReadCounter
export _TempCounter_wReadCounter

; The following functions are deprecated and subject to omission in future releases
;
export  wTempCounter_ReadCompareValue  ; deprecated
export _wTempCounter_ReadCompareValue  ; deprecated
export  wTempCounter_ReadCounter       ; deprecated
export _wTempCounter_ReadCounter       ; deprecated
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: TempCounter_EnableInt
;
;  DESCRIPTION:
...
_TempCounter_EnableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   TempCounter_EnableInt_M
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: TempCounter_DisableInt
;
;  DESCRIPTION:
...
;    functions.
;
 TempCounter_DisableInt:
_TempCounter_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   TempCounter_DisableInt_M
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: TempCounter_Start
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: TempCounter_Stop
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: TempCounter_WritePeriod
;
;  DESCRIPTION:
...
;    functions.
;
 TempCounter_WritePeriod:
_TempCounter_WritePeriod:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[TempCounter_PERIOD_LSB_REG], A
   mov   A, X
   mov   reg[TempCounter_PERIOD_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: TempCounter_WriteCompareValue
;
;  DESCRIPTION:
...
;    functions.
;
 TempCounter_WriteCompareValue:
_TempCounter_WriteCompareValue:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[TempCounter_COMPARE_LSB_REG], A
   mov   A, X
   mov   reg[TempCounter_COMPARE_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: TempCounter_wReadCompareValue
;
;  DESCRIPTION:
...
;    functions.
;
 TempCounter_wReadCompareValue:
_TempCounter_wReadCompareValue:
 wTempCounter_ReadCompareValue:                  ; this name deprecated
_wTempCounter_ReadCompareValue:                  ; this name deprecated
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   A, reg[TempCounter_COMPARE_MSB_REG]
   mov   X, A
   mov   A, reg[TempCounter_COMPARE_LSB_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: TempCounter_wReadCounter
;
;  DESCRIPTION:
...
;        functions.
;
 TempCounter_wReadCounter:
_TempCounter_wReadCounter:
 wTempCounter_ReadCounter:                       ; this name deprecated
_wTempCounter_ReadCounter:                       ; this name deprecated
   ...
   mov   A, reg[TempCounter_COMPARE_MSB_REG]     ; Save the Compare register on the stack
   push  A                                          ;
   mov   A, reg[TempCounter_COMPARE_LSB_REG]     ;
   push  A                                          ;   -stack frame now 2 bytes-
   mov   A, reg[TempCounter_CONTROL_LSB_REG]     ; Save CR0 (running or stopped state)
   push  A                                          ;   -stack frame now 3 bytes-
   TempCounter_Stop_M                            ; Disable (stop) the Counter if running
   M8C_SetBank1                                     ;
   mov   A, reg[TempCounter_INPUT_LSB_REG]       ; save the LSB clock input setting
   push  A                                          ;   on the stack (now 4 bytes) and ...
                                                    ;   hold the clock low:
   mov   reg[TempCounter_INPUT_LSB_REG], INPUT_REG_NULL
   M8C_SetBank0                                     ; Extract the Count via DR2 register
   mov   A, reg[TempCounter_COUNTER_MSB_REG]     ; DR2 <- DR0 (in the MSB block)
   mov   A, reg[TempCounter_COMPARE_MSB_REG]     ; Stash the Count MSB on the stack
   push  A                                          ;   -stack frame is now 5 bytes
   mov   A, reg[TempCounter_COUNTER_LSB_REG]     ; DR2 <- DR0 (in the LSB block)
   mov   A, reg[TempCounter_COMPARE_LSB_REG]     ; Stash the Count LSB on the stack
   ...
   mov   reg[TempCounter_INPUT_LSB_REG], A       ;   and restore it
   M8C_SetBank0                                     ; Now re-enable (start) the counter
   mov   A, [X+bOrigControlReg]                     ;   if it was running when
   mov   reg[TempCounter_CONTROL_LSB_REG], A     ;   this function was first called
