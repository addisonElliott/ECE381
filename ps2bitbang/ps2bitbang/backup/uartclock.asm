...
include "m8c.inc"
include "memory.inc"
include "UARTClock.inc"

;-----------------------------------------------
;  Global Symbols
;-----------------------------------------------
export  UARTClock_EnableInt
export _UARTClock_EnableInt
export  UARTClock_DisableInt
export _UARTClock_DisableInt
export  UARTClock_Start
export _UARTClock_Start
export  UARTClock_Stop
export _UARTClock_Stop
export  UARTClock_WritePeriod
export _UARTClock_WritePeriod
export  UARTClock_WriteCompareValue
export _UARTClock_WriteCompareValue
export  UARTClock_wReadCompareValue
export _UARTClock_wReadCompareValue
export  UARTClock_wReadCounter
export _UARTClock_wReadCounter

; The following functions are deprecated and subject to omission in future releases
;
export  wUARTClock_ReadCompareValue  ; deprecated
export _wUARTClock_ReadCompareValue  ; deprecated
export  wUARTClock_ReadCounter       ; deprecated
export _wUARTClock_ReadCounter       ; deprecated
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UARTClock_EnableInt
;
;  DESCRIPTION:
...
;
 UARTClock_EnableInt:
_UARTClock_EnableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   UARTClock_EnableInt_M
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UARTClock_DisableInt
;
;  DESCRIPTION:
...
;    functions.
;
 UARTClock_DisableInt:
_UARTClock_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   UARTClock_DisableInt_M
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UARTClock_WritePeriod
;
;  DESCRIPTION:
...
;    functions.
;
 UARTClock_WritePeriod:
_UARTClock_WritePeriod:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[UARTClock_PERIOD_LSB_REG], A
   mov   A, X
   mov   reg[UARTClock_PERIOD_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UARTClock_WriteCompareValue
;
;  DESCRIPTION:
...
;    functions.
;
 UARTClock_WriteCompareValue:
_UARTClock_WriteCompareValue:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[UARTClock_COMPARE_LSB_REG], A
   mov   A, X
   mov   reg[UARTClock_COMPARE_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UARTClock_wReadCompareValue
;
;  DESCRIPTION:
...
;    functions.
;
 UARTClock_wReadCompareValue:
_UARTClock_wReadCompareValue:
 wUARTClock_ReadCompareValue:                    ; this name deprecated
_wUARTClock_ReadCompareValue:                    ; this name deprecated
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   A, reg[UARTClock_COMPARE_MSB_REG]
   mov   X, A
   mov   A, reg[UARTClock_COMPARE_LSB_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UARTClock_wReadCounter
;
;  DESCRIPTION:
...
;        functions.
;
 UARTClock_wReadCounter:
_UARTClock_wReadCounter:
 wUARTClock_ReadCounter:                         ; this name deprecated
_wUARTClock_ReadCounter:                         ; this name deprecated
   ...
   mov   A, reg[UARTClock_COMPARE_MSB_REG]       ; Save the Compare register on the stack
   push  A                                          ;
   mov   A, reg[UARTClock_COMPARE_LSB_REG]       ;
   push  A                                          ;   -stack frame now 2 bytes-
   mov   A, reg[UARTClock_CONTROL_LSB_REG]       ; Save CR0 (running or stopped state)
   push  A                                          ;   -stack frame now 3 bytes-
   UARTClock_Stop_M                              ; Disable (stop) the Counter if running
   M8C_SetBank1                                     ;
   mov   A, reg[UARTClock_INPUT_LSB_REG]         ; save the LSB clock input setting
   push  A                                          ;   on the stack (now 4 bytes) and ...
                                                    ;   hold the clock low:
   mov   reg[UARTClock_INPUT_LSB_REG], INPUT_REG_NULL
   M8C_SetBank0                                     ; Extract the Count via DR2 register
   mov   A, reg[UARTClock_COUNTER_MSB_REG]       ; DR2 <- DR0 (in the MSB block)
   mov   A, reg[UARTClock_COMPARE_MSB_REG]       ; Stash the Count MSB on the stack
   push  A                                          ;   -stack frame is now 5 bytes
   mov   A, reg[UARTClock_COUNTER_LSB_REG]       ; DR2 <- DR0 (in the LSB block)
   mov   A, reg[UARTClock_COMPARE_LSB_REG]       ; Stash the Count LSB on the stack
   ...
   mov   reg[UARTClock_INPUT_LSB_REG], A         ;   and restore it
   M8C_SetBank0                                     ; Now re-enable (start) the counter
   mov   A, [X+bOrigControlReg]                     ;   if it was running when
   mov   reg[UARTClock_CONTROL_LSB_REG], A       ;   this function was first called
