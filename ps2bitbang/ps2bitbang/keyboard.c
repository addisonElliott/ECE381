...
char KeyboardToASCII(BYTE scancode, BYTE codetype)
{
	...
	if (codetype & KEY_MAKE) {
		...
		if (codetype & KEY_E0) {
			if (scancode == 0x71 && !CtrlDown && !AltDown) return(0x08);	// Delete (return backspace)
...
void KeyboardAlt(BYTE *str, BYTE index)
{
	if (index == 3)
	{
		if (str[0] == '3' && str[1] == '1' && str[2] == '4') UART_CPutString("3.1415926535897932386\r\n");
		else if (str[0] == '0' && str[1] == '0' && str[2] == '0') UART_CPutString("I love ECE381\r\n");
		else if (str[0] == '1' && str[1] == '2' && str[2] == '3') UART_CPutString("The quick brown fox jumps over the lazy dog\r\n");
	}
}
	...
	if (codetype) {
		...
		if ((codetype & KEY_E0)&&(codetype & KEY_BREAK)) {
			...
			case 0x11:	// E0 F0 11 - Right Alt
				...
				if (!altstatus)
				{	
					KeyboardAlt(altStr, altIndex);
					AltDown = 0;
					altIndex = 0;
				}
				break;
			case 0x71:	// E0 F0 71 - Delete
				if (CtrlDown && AltDown)
					M8C_Reset;	
				break;		
		...
		if (codetype & KEY_MAKE) {
			if (AltDown){
				...
				if (!NumLock) {
					...
					if (KeypadScancode[i][1] > 0 && altIndex < 11) {
						altStr[altIndex++] = KeypadScancode[i][0];
					}
				}
				i = 0;
				while((ShiftableScancode[i][2] != scancode) && (ShiftableScancode[i][2] != 0)) {
				...
				if (ShiftableScancode[i][2] > 0 && altIndex < 11) {
					altStr[altIndex++] = ShiftableScancode[i][ShiftDown];
				}
			}
			switch(scancode) {
			...
			case 0x05:	// F1 - Dim the LED
				...
				else if (brightindex <= 32 && brightindex != 0){
					brightindex = 0;
					PWM8LED_Stop();
				}
				break;
