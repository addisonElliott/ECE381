...
void clock10us_Start(void) 
{
    ...
    clock10us_CLKEN |= clock10us_CLKEN_MASK;
	clock10us_CLKSTBY |= clock10us_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clock10us_Stop
********************************************************************************
*
* Summary:
...
void clock10us_Stop(void) 
{
    ...
    clock10us_CLKEN &= (uint8)(~clock10us_CLKEN_MASK);
	clock10us_CLKSTBY &= (uint8)(~clock10us_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clock10us_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clock10us_CLKEN & clock10us_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clock10us_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clock10us_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clock10us__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clock10us_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clock10us_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clock10us_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clock10us_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clock10us_GetSourceRegister();
    uint16 oldDivider = clock10us_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clock10us_CLKEN & clock10us_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clock10us_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clock10us_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clock10us_DIV_PTR);
}


/*******************************************************************************
* Function Name: clock10us_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clock10us_MOD_SRC |= modeBitMask & (uint8)clock10us_MODE_MASK;
}


/*******************************************************************************
* Function Name: clock10us_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clock10us_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clock10us_MODE_MASK));
}


/*******************************************************************************
* Function Name: clock10us_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clock10us_MOD_SRC & (uint8)(clock10us_MODE_MASK);
}


/*******************************************************************************
* Function Name: clock10us_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clock10us_GetDividerRegister();
    uint8 oldSrc = clock10us_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clock10us_MOD_SRC |= CYCLK_SSS;
        clock10us_MOD_SRC =
            (clock10us_MOD_SRC & (uint8)(~clock10us_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clock10us_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clock10us_MOD_SRC & clock10us_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clock10us_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clock10us_PHASE = clkPhase & clock10us_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clock10us_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clock10us_PHASE & clock10us_PHASE_MASK;
}
