...
void mainClock_Start(void) 
{
    ...
    mainClock_CLKEN |= mainClock_CLKEN_MASK;
	mainClock_CLKSTBY |= mainClock_CLKSTBY_MASK;
}
...
void mainClock_Stop(void) 
{
    ...
    mainClock_CLKEN &= (uint8)(~mainClock_CLKEN_MASK);
	mainClock_CLKSTBY &= (uint8)(~mainClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: mainClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((mainClock_CLKEN & mainClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = mainClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = mainClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* mainClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(mainClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(mainClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: mainClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: mainClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = mainClock_GetSourceRegister();
    uint16 oldDivider = mainClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = mainClock_CLKEN & mainClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((mainClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(mainClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: mainClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    mainClock_MOD_SRC |= modeBitMask & (uint8)mainClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: mainClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    mainClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(mainClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: mainClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return mainClock_MOD_SRC & (uint8)(mainClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: mainClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = mainClock_GetDividerRegister();
    uint8 oldSrc = mainClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        mainClock_MOD_SRC |= CYCLK_SSS;
        mainClock_MOD_SRC =
            (mainClock_MOD_SRC & (uint8)(~mainClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: mainClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return mainClock_MOD_SRC & mainClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: mainClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    mainClock_PHASE = clkPhase & mainClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: mainClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return mainClock_PHASE & mainClock_PHASE_MASK;
}
