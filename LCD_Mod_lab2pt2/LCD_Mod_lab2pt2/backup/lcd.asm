...
LCD_WDATA_Nibble:
    ...
    RAM_SETPAGE_CUR >NO_SHADOW          ; Set CUR_PP to LCD variable address
    and   [NO_SHADOW],~LCD_PORT_MASK
    or    [NO_SHADOW],LCD_RS                          ; Raise RS to signify a Data Write
    mov   A,[NO_SHADOW]
    mov   reg[LCD_Port],A
...
LCD_Check_Ready:
    ...
    RAM_SETPAGE_CUR >NO_SHADOW                        ; Set CUR_PP to LCD variable address
    and   [NO_SHADOW],~LCD_PORT_MASK                  ; Mask of all LCD bits
    mov   A,[NO_SHADOW]
    mov   reg[LCD_Port],A                             ; Zero LCD port bits

    and   [NO_SHADOW],~LCD_DATA_MASK                  ; Clear out LCD mode bits.
    mov   A,[NO_SHADOW]
    M8C_SetBank1                                      ; Change port mode to read status
    mov   reg[LCD_PortMode0],A                        ; Setup LCD Port for reading
    M8C_SetBank0

    or    [NO_SHADOW],LCD_RW                          ; Raise RW to signify Read operation
    mov   A,[NO_SHADOW]
    mov   reg[LCD_Port],A
    NOP
...
.LCD_RDY_LOOP:
    or    [NO_SHADOW], LCD_CNTL_READ                            ; Raise E to start cycle
    mov   A,[NO_SHADOW]
    mov   reg[LCD_Port],A
...
ENDIF   
    ...
    push  A
    and   [NO_SHADOW],(~LCD_PORT_MASK | LCD_RW)                 ; Lower E signal
    mov   A,[NO_SHADOW]
    mov   reg[LCD_Port],A
...
.UNLOCK:
    ...
    or    [NO_SHADOW],LCD_PORT_WRITE                  ; Revert Data bit to Write mode
    mov   A,[NO_SHADOW]
    M8C_SetBank1
...
_LCD_Init:
    ...
    and   [NO_SHADOW],~LCD_PORT_MASK                            ; Mask off LCD bits
    or    [NO_SHADOW],LCD_PORT_WRITE                            ; Set LCD port for writing
    and   [NO_SHADOW],~LCD_PORT_MASK                            ; Mask off LCD bits
